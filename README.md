...
| 提示词 | ✅ | 一键发送系统提示词 |
| 静默刷新 | ✅ | 文件操作后自动刷新文件树 |
| 代码匹配 | ✅ | 精确匹配 + 模糊匹配（保守策略，宁可不匹配也不错位） |
| 状态持久化 | ✅ | 页面刷新后保留已应用状态 |
| 依赖分析 | ✅ | 自动解析 import/require，一键发送文件及依赖 |
| JS 语法检查 | ✅ | 应用补丁前检查括号匹配，防止错误代码被应用 |
| 按钮防重复 | ✅ | 应用修改时禁用按钮，防止重复点击 |
| 目录树美化 | ✅ | 带连接线的树形结构（├── └──） |
...
| patcher.js | ~150 | 代码匹配算法（精确/模糊）+ JS 语法检查（括号匹配） |
| state.js | ~80 | 补丁状态持久化（localStorage + 模糊匹配检测） |
